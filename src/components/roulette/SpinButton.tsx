...
const SpinButton = memo(({isSpinning, onSpin, price}: SpinButtonProps) => {
    return (
        <Button
            onClick={onSpin}
            disabled={isSpinning}
            className="w-full py-6 px-6 rounded-xl font-medium flex items-center justify-center gap-1 transition-all duration-300 bg-gradient-to-r bg-customPurple text-white text-base hover:opacity-90 disabled:opacity-50 h-auto"
        >
            {isSpinning ? (
                <>
