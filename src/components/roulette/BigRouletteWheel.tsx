...
import {prizeGridDataBig} from "@/data/rouletteData.ts";
...
const BigRouletteWheel = ({items: initialItems, onSpin, onPlayAgain, userData}: RouletteWheelProps) => {
    ...
    const spinWheel = async () => {
        ...
        try {
            const response = await apiService.createBigDefaultGiftSpin();
    ...
    return (
        <div className="flex flex-col items-center">
            <StarCard className="relative w-full max-w-md mb-6 p-6">
                ...
                <div className="mt-6">
                    <SpinButton isSpinning={isSpinning} onSpin={spinWheel} price=" за 100"/>
                </div>
            </StarCard>

            {/* Показываем все возможные выигрыши под рулеткой */}
            <PrizeGrid items={prizeGridDataBig} onItemClick={handleItemClick}/>
...
export default BigRouletteWheel;
