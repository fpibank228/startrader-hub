...
const NftResultModal = ({ isOpen, onClose, result, onPlayAgain }: RouletteResultModalProps) => {
  if (!result) {
    return null;
  }
      ...
      <DialogContent className="bg-gradient-to-b bg-customDark border-white/40 border-2 p-0 overflow-hidden max-w-sm w-full rounded-2xl ">
        ...
        <div className="flex flex-col items-center p-4">
          ...
          >
            ...
            {result.price && (
                <p className="font-medium text-lg flex items-center justify-center">{result.price}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
            ...
            {onPlayAgain && (
                <Button
                    onClick={onPlayAgain}
                    className="bg-gradient-to-r customBlue mx-5 py-5 h-auto flex items-center justify-center"
                >
                  Продать
                </Button>
            )}
